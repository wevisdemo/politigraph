...
export const queryFieldTypenameMap = new Map(
	searchableTypes.map(({ name }) => [getQueryFieldName(name), name]),
);
...
const searchQuery = `query Search($keyword: String!) {
	${searchableTypes
		.map(({ name, searchFields }) => {
...
export function buildCenterNodeQuery(typename: string) {
	const cachedQuery = centerNodeQueryCache.get(typename);
	if (cachedQuery) return cachedQuery;

	const query = `query CenterNode($id: ID!) {
	...
	centerNodeQueryCache.set(typename, query);
