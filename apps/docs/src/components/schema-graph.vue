			...
			<template v-if="selectedNode">
				...
				<div>
					<div class="flex flex-row items-center gap-2">
						<h3 class="text-xl">{{ selectedNode.name }}</h3>
						<div
				...
				<ul
					...
					class="ml-3 mt-0 flex list-disc flex-col gap-2 p-0 text-sm"
				>
				...
				>
					...
					>
						...
						>
							<li
								v-for="{ name, description } in enums.find(
									(e) => e.name === type.name,
								)!.values"
								:key="name"
							>
