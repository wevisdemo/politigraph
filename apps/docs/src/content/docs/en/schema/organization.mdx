...
<QueryTabs
	query="query Query($where: OrganizationWhere) { organizations(where: $where) { id name_en classification founding_date dissolution_date children { id name_en classification description founding_date dissolution_date } } }"
...
<QueryTabs
	query="query Organizations($where: OrganizationWhere) { organizations(where: $where) { id name name_en color } }"
